...
pub async fn post_new_course_db(
    ...
    let row = sqlx::query_as!(
        ...
        RETURNING id,time,teacher_id,name,description,format,structure,duration,price,language,level"#,
...
pub async fn update_course_details_db(
    ...
    let course_row = sqlx::query_as!(
        Course,
        "UPDATE course SET name=$1, description=$2, format=$3, structure=$4,
        duration=$5, price=$6, language=$7, level=$8 WHERE teacher_id=$9 AND id=$10
        RETURNING id,time,teacher_id,name,description,format,structure,duration,price,language,level",
        name,
        description,
        format,
